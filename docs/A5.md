...
| **Keys**        | { id } |
| **Functional Dependencies:** |       |
| FD0701          | id → {body, date, title, trending_score, tag_id} |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id } |
| **Functional Dependencies:** |       |
| FD0801          | id → {content_id, body, date} |
| **NORMAL FORM** | BCNF               |
...
CREATE TABLE "ban"(
    ...
    reason text,
    moderator_id INTEGER NOT NULL REFERENCES "moderator" (id) ON UPDATE CASCADE,
