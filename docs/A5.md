...
| ------------------ | ------------------------------------------------ |
| R01                | users(**id**, username UK NN, email UK NN, password, description, photo UK, birthdate NN CK years(TODAY - birthdate) >= 13, gender NN CK gender IN GENDER_TYPE, reputation NN DF 0 CK reputation >= 0, is_moderator NN DF false, is_partner NN DF false, is_banned NN DF false,is_deleted NN DF false) |
| R02                | follow(**follower_id** → users NN, **users_id** → users NN)
| R03                | ban(**id**, users_id → users NN, moderator_id → users NN, start NN DF TODAY, end DF NULL CK end > start, reason NN) |
| R04                | content(**id**, author_id → users, body NN, date NN DF TODAY, nr_votes NN DF 0) |  
...
| R15                | follow_notification(**follower_id** → users NN, **users_id** → vote, is_new DF true, date DF TODAY) |
| R16                | vote_notification(**vote_id** → users, **content_id** → content, **author_id** → users NN, is_new DF true, date DF TODAY) |
| R17                | comment_notification(**users_id** → users, **comment_id** → comment, is_new DF true, date DF TODAY) |
...
| **Keys**        | {id} |
| **Functional Dependencies:** |
| FD0501          | id → {users_id, moderator_id, start, end, reason} |
| **NORMAL FORM** | BCNF |
...
| **Keys**        | {follower_id, users_id} |
| **Functional Dependencies:**  |
| FD0301          | follower_id, users_id → {is_new, date} |
| **NORMAL FORM** | BCNF |

| **TABLE R16**   | vote_notification |
| --------------  | --- |
| **Keys**        | {vote_id, content_id, author_id} |
| **Functional Dependencies:**  |
| FD0301          | vote_id, content_id, author_id → {is_new, date} |
| **NORMAL FORM** | BCNF |
...
| **Functional Dependencies:**  |
| FD0301          | users_id, comment_id → {is_new, date} |
| **NORMAL FORM** | BCNF |
