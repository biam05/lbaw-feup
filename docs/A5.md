...
| R03                | ban(**id**, users_id → users NN, moderator_id → moderator, start NN DF TODAY, end DF NULL CK end > start, reason NN) |
...
| R16                | vote_notification(**voter_id** → users, **content_id** → content, **author_id** → users NN, is_new DF true, date DF TODAY) |
...
| **Keys**        | {id} |
| **Functional Dependencies:** |
| FD0401          | id → {users_id, content_id, value} |
| **NORMAL FORM** | BCNF |
...
| **Keys**        | {id} |
| **Functional Dependencies:**  |
| FD0301          | id → {users_id, vote_id, comment_id, is_new} |
| **NORMAL FORM** | BCNF |
