...
| **Description**  | A user cannot follow himself |
 ```sql
  CREATE FUNCTION follow_self() RETURNS TRIGGER AS
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM follow WHERE follower_id=users_id) THEN
...
| **Description**  | A User is only able to request the label of Partner when it reaches of 100000 |
 ```sql
  CREATE FUNCTION make_partner_request() RETURNS TRIGGER AS
  $BODY$
        BEGIN
...
| **Description**  | The minimum age for a user to be registers is 13 years old |
 ```sql
  CREATE FUNCTION minimum_age() RETURNS TRIGGER AS
    $BODY$
        New.birthdate >= now() - INTERVAL '13 years';
    $BODY$
  LANGUAGE plpgsql;
...
| **Description**  | An Authenticated User can't vote on his own news/comments. |
 ```sql
  CREATE FUNCTION vote_self() RETURNS TRIGGER AS
  $BODY$
  BEGIN
...
-- Create vote
INSERT INTO vote (users_id, content_id, value)
VALUES ($users_id, $content_id, $value);

-- Update content
UPDATE content SET content.nr_votes=content.nr_votes+$value;
WHERE content.id=$content_id;

-- Update news
UPDATE news SET news.trending_score=news.trending_score+$value;
WHERE news.content_id=$content_id;
