...
| **Type**            | Hash |
| **Clustering**      | Yes |
| **Justification**   | To allow searching the first 25 news of a certain trending score that have the trending score lower than a certain value faster; It’s B-tree and clustered to allow for quick range queries.|
 ```sql
...
| **Type**            | Hash |
| **Clustering**      | Yes |
| **Justification**   | To allow searching the first 25 news of a certain date that have the date lower than a certain value faster; It’s B-tree and clustered to allow for quick range queries.|
 ```sql
...
| **Type**            | Hash |
| **Clustering**      | Yes |
| **Justification**   | To allow searching the first 25 news wirth a certain amount of votes that have the number of votes lower than a certain value faster; It’s B-tree and clustered to allow for quick range queries.|
 ```sql
...
CREATE INDEX search_news_idx ON news USING GIST (to_tsvector('english', title));
 ```
...
| **Justification**   | Used to improve performance of full text searches when searching for users, gin because users data is rarely changed. |
 ```sql
CREATE INDEX search_username_idx ON users USING GIN (to_tsvector('english', username));
 ```
...
| **Description**  | A user cannot follow himself |
 ```sql
  ...
  $BODY$
  BEGIN
    IF NEW.follower_id=NEW.user_id THEN
...
| **Trigger**      | TRIGGER04                             |
| ---              | ---                                    |
| **Description**  | A User is only able to request the label of Partner when it reaches of 100000 |
 ```sql
  CREATE OR REPLACE FUNCTION make_partner_request() RETURNS TRIGGER AS
  $BODY$
        BEGIN
            IF (SELECT * FROM users WHERE users.id = New.partner_id AND users.reputation < 100000)
                THEN RAISE EXCEPTION 'A User is only able to request the label of Partner when it reaches of 100000';
            END IF;
            RETURN New;
        END
    $BODY$
  ...
  CREATE TRIGGER make_partner_request
    BEFORE INSERT ON partner_request
    FOR EACH ROW
    EXECUTE PROCEDURE make_partner_request();
...
| **Description**  | An Authenticated User can't vote on his own news/comments. |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM vote, content WHERE users_id=auhtor_id AND content_id = content.id) THEN
...
| **Description**  | Increase Number of Comments in a News Post |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM news WHERE new.news_id=news.content_id) THEN
        news.nr_comment = news.nr_comments + 1; 
    END IF;
    RETURN new;
  END
...
| **Description**  | Decrease Number of Comments in a News Post |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM news WHERE old.news_id=news.content_id) THEN
        news.nr_comment = news.nr_comments - 1; 
    END IF;
    RETURN old;
  END
...
| **Description**  | Increase Trending Score and Number of Votes with a Vote |
 ```sql
  ...
  $BODY$
  BEGIN
        UPDATE news SET trending_score = trending_score + new.value WHERE news.content_id=new.content_id;
        
        UPDATE news SET nr_votes = nr_votes + new.value
...
| **Description**  | Decrease Trending Score and Number of Votes with a Vote|
 ```sql
  ...
  BEGIN
        ...
        UPDATE news SET trending_score = news.trending_score - old.value
        WHERE old.content_id=news.content_id ;
        UPDATE news SET nr_votes = nr_votes - old.value
        WHERE old.content_id=news.content_id ;
...
| **Description**  | Create Follow Notification |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM users WHERE new.users_id=users.id) THEN
...
| **Description**  | Create Vote Notification |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM users, content WHERE new.users_id=users.id AND new.content_id=content.id) THEN
        INSERT INTO vote_notification
            VALUES (new.users_id, content.id, content.author_id, true, now());
    END IF;
    RETURN new;
  END  
  $BODY$
...
| **Description**  | Create Comment Notification |
 ```sql
  ...
  $BODY$
  BEGIN
    IF EXISTS (SELECT * FROM content WHERE new.content_id=content.id) THEN
        INSERT INTO comment_notification
            VALUES (content.author_id, new.content_id, true, now());
    END IF;
    RETURN new;
  END  
  $BODY$
...
| **Trigger**      | TRIGGER16                            |
| ---              | ---                                    |
| **Description**  | Updatr TSVECTOR (Users) |
 ```sql
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        IF new.username <> new.username THEN
            new.search = to_tsvector('english', new.username);
        END IF;
    END IF;
END  
