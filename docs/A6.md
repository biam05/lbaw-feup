...
```sql
    ...
    ORDER BY follow_notification.creation_date DESC;

    SELECT vote_notification.voter_id, vote_notification.content_id, vote_notification.is_new, vote_notification.creation_date
    FROM vote_notification
    WHERE vote_notification.author_id = $my_users_id
    ORDER BY vote_notification.creation_date DESC;   
    
    SELECT comment_notification.comment_id, comment_notification.is_new, comment_notification.creation_date
    FROM comment_notification
    WHERE comment_notification.users_id = $my_users_id
    ORDER BY comment_notification.creation_date DESC;
```
